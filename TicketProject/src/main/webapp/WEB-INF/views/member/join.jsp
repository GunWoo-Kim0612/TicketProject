...
<link rel="stylesheet" href="https://tis-teamproject.s3.ap-northeast-2.amazonaws.com/css/style.css">
    		...
    		if(id.length >= 6 && id.length <= 16){
	    		$.ajax({
		        	...
		        	success : function(data){
		        		if(data.result == "1"){
			        		$('#chkId').html('(중복확인)아이디 생성 가능!');
			        		$('#chkId').css('color','#00ff00');
		        		}else{
			        		$('#chkId').html('(중복확인)중복 아이디. 아이디로 사용하실수 없습니다!');
			        		$('#chkId').css('color','#ff0000');
		        		}
		        		
		        	},
		        	error : function(){
		        		$('#chkId').html('(중복확인)서버 요청 실패...ㅠㅠ');
		        	}
		        });
    		}else{
    			$('#chkId').html('(중복확인) 아이디는 6자 이상 16자 이하 로 적어주세요!');
    			$('#chkId').css('color', 'orange');
    		}
  ...
  <body>
    ...
    <jsp:include page="../mypage/mypage_b.jsp"></jsp:include>
    <div class="container" style="width: 800px">
        <form action="/updateMember.do" class="mt-5" method="post" name="edit" onsubmit="return chkAble();">
	      	<div class="card mb-3 mt-3 shadow">
	          <h2 class="card-body"><span class="badge bg-dark shadow">회원가입</span></h2>
	          <div class="form-group card-body">
	            <label for="user_name" class="form-label mt-4">이름</label>
	            <input type="text" class="form-control" id="user_name" name="name" placeholder="이름을 입력해주세요."/>
	          </div>
	          <div class="form-group card-body">
	            <label for="userId" class="form-label mt-4" >아이디 </label>
	            <span id="chkId">${msg}</span>
	            <input type="text" class="form-control" id="userId" name="id" placeholder="아이디를 입력해주세요." onkeyup="chk_id();"/>
	          </div>
	          <div class="form-group card-body">
	            <label for="userPw" class="form-label mt-4">비밀번호</label>
	            <input type="password" class="form-control" id="userPw" name="pw" placeholder="비밀번호를 입력해주세요." onkeyup="cf_pw();"/>
	          </div>
	          <div class="form-group card-body">
	          ...
	          <div class="form-group card-body">
	            ...
	            <input type="text" class="form-control" id="userPh" name="phone"  placeholder="전화번호를 입력해주세요." />
	          </div>
	          <div class="mt-4 mb-4" style="text-align: center">
	            <button class="btn btn-secondary" style="width: 300px">회원 가입</button>
	          </div>
		    </div>
        </form>
      </div>
